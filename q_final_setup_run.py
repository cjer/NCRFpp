...
for conf in os.scandir('final_setup/noseg_plo_conf'):
    if conf.name.startswith(prefix):
        model_base_name = '.'.join(conf.name.split('.')[:-1])
        log_path = os.path.join('final_setup/noseg_plo_logs', model_base_name+'.log')
        dset_path = os.path.join('final_setup/noseg_plo_models', model_base_name+'.dset')
